...
tdDataUpload <- function(connection, connection_fast = NULL, data.base.name = NULL, table.name, data, replace = T, fast = F, primaryIndex = NULL , partitionDate = NULL, volatile = F, na.default = NULL) {
  ...
  if(fast == T){
    ...
    if(tdExistsTable(data.base, paste0(table.name,"_err_1"), conn)) dbSendUpdate(conn, paste0("drop table ",target.table,"_err_1"))
    if(tdExistsTable(data.base, paste0(table.name,"_err_2"), conn)) dbSendUpdate(conn, paste0("drop table ",target.table,"_err_2"))    
    if(tdExistsTable(data.base, paste0(table.name,"_ERR_1"), conn)) dbSendUpdate(conn, paste0("drop table ",target.table,"_ERR_1"))
    if(tdExistsTable(data.base, paste0(table.name,"_ERR_2"), conn)) dbSendUpdate(conn, paste0("drop table ",target.table,"_ERR_2"))
